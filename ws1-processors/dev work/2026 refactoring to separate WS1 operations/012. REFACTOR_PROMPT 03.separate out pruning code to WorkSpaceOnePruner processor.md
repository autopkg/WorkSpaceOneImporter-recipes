...
- ✅ Update `WorkSpaceOneSlacker` to read both `ws1_importer_summary_result` and
  `ws1_pruner_summary_result`. > already done > in `WorkSpaceOneSlacker.py` lines 50–56.
- ⛔️ ~~Add a `ws1_app_name` input variable as an explicit override (instead of
  relying solely on `NAME`) for cases where the WS1 app name differs from the
  munki name.~~ Not applicable as `NAME` is already used as the app name in the importer and is always set by the parent recipe. The pruner can just read `NAME` for consistency. If a separate override variable is added in the future, it should default to `NAME` if not set.
